...
def torch_matches(tag: str, target: str, fmt: str = "minor") -> bool:
    ...
    if fmt == "minor":
        return tag == target_m
    if fmt == "packed":
        # NATTEN-style: 2110=2.11, 280=2.8 (major + minor*10 + patch)
        d = tag
        if len(d) == 4:
        ...
        return tag == target_m
    if fmt == "pyg":
        # PyG-style: 210=2.10, 211=2.11, 25=2.5 (just major + minor concatenated)
        d = tag
        major = d[0]
        minor = d[1:]
...
def _normalize_torch_display(tv_raw: str, fmt: str) -> Optional[str]:
    ...
    if fmt == "packed":
        ...
        tv = f"{tv_raw[0]}.{tv_raw[1:]}" if len(tv_raw) >= 2 else tv_raw
    elif fmt == "full":
