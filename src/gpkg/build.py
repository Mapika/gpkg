...
def ensure_ninja() -> bool:
    """Ensure ninja build system is available in the project venv."""
    python = _find_project_python()
    # Check if ninja is importable in the project venv
    try:
        result = subprocess.run(
        ...
        pass
    # Install ninja into the project venv
    pip_cmd = _find_pip_command()
    if not pip_cmd:
        return False
    try:
        subprocess.run(
            [*pip_cmd, "install", "ninja", "--quiet"],
...
def _find_project_python() -> Optional[str]:
    """Find the project venv's Python interpreter.

    When gpkg is installed as a uv tool, sys.executable points to gpkg's own
    isolated venv — not the project's.  We need the project venv's Python so
    that --no-build-isolation can find torch and other build deps.
    """
    ...
    # 2. .venv in the current directory (uv sync default)
    candidate = Path.cwd() / ".venv" / "bin" / "python"
    if candidate.exists():
        return str(candidate)
    # 3. Fall back to sys.executable (works if gpkg is run inside the project venv)
    return sys.executable


def _find_pip_command() -> list[str]:
    """Find a working pip command that runs inside the project venv.
    ...
    try:
        ...
        pass
    # Try installing pip into the project venv via uv
